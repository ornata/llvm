...
public:
  ...
  /// \param FunctionList Filled with a list of \p OutlinedFunctions each type
  /// of candidate.
  /// \param BenefitFn The function to satisfy.
  ///
  /// \returns The length of the longest candidate found.
  size_t findAllCandidates(std::vector<Candidate> &CandidateList,
  std::vector<OutlinedFunction> &FunctionList,
  const std::function<unsigned(SuffixTreeNode &, size_t)> &BenefitFn) {
  ...
  for (auto It = CandidateList.begin(), Et = CandidateList.end(); It != Et;
    ...
    for (auto Sit = It + 1; Sit != Et; Sit++) {
      ...
      if (C1.Benefit >= C2.Benefit) {
        // Update the function's occurrence count and benefit to reflect that C2
        // is being removed.
        F2.OccurrenceCount--;
