...
X86InstrInfo::X86InstrInfo(X86Subtarget &STI)
  ...
  static const X86MemoryFoldTableEntry MemoryFoldTable1[] = {
      ...
      // instruction on AVX1 targets. The VPBROADCAST instructions are all AVX2
      // instructions so they don't need an equivalent limitation.
  ...
  case X86::DEC32r: {
    ...
    MachineInstrBuilder MIB =
        BuildMI(MF, MI.getDebugLoc(), get(Opc))
            .add(Dest)
            .addReg(SrcReg,
                    getUndefRegState(isUndef) | getKillRegState(isKill));
    if (ImplicitOp.getReg() != 0)
      MIB.add(ImplicitOp);
...
static bool commuteVPTERNLOG(MachineInstr &MI, unsigned SrcOpIdx1,
  ...
  uint8_t NewImm = Imm &
                   ~(SwapMasks[Case][0] | SwapMasks[Case][1] |
...
bool X86InstrInfo::unfoldMemoryOperand(
  ...
  for (MachineOperand &ImpOp : ImpOps) {
    MIB.addReg(ImpOp.getReg(),
               getDefRegState(ImpOp.isDef()) | RegState::Implicit |
